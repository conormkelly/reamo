...
async function renderPeaksToBitmap(
  ...
  const lCenterY = stereo ? height / 4 : height / 2;
  const rCenterY = (3 * height) / 4;
  ...
  for (let i = 0; i < peaks.length; i++) {
    ...
    if (stereo) {
      ...
      ctx.fillRect(x, lAdjustedTop, barWidth, lHeight);
      ...
      ctx.fillRect(x, rAdjustedTop, barWidth, rHeight);
    } else {
      ...
      ctx.fillRect(x, adjustedTopY, barWidth, peakHeight);
    }
  }
