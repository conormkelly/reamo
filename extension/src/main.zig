...
// =============================================================================
// Lua Bridge POC: Shared memory for peak data transfer
// =============================================================================
// Lua Peak Bridge - for receiving peak data from Lua scripts.
// Peak fetching is done via Lua bridge for reliable results across platforms.

const LuaPeakBridge = struct {
    ...
    pub fn register(plugin_register: *const fn ([*:0]const u8, ?*anyopaque) callconv(.c) c_int) void {
...
fn doInitialization() !void {
    ...
    if (g_api) |*lua_api| {
        if (LuaPeakBridge.initScript(lua_api)) {
            // Register the bridge function with peaks_generator
            peaks_generator.setLuaBridgeFn(&LuaPeakBridge.bridgeFetchAdapter);
        }
    }
