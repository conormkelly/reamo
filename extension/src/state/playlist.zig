...
test "Playlist serialize/deserialize" {
    ...
    try std.testing.expectEqualStrings("Friday Gig|S:1|1,4|2,2|3,1", serialized);
...
test "Playlist name with pipe escaping" {
    ...
    var buf: [1024]u8 = undefined;
    const serialized = p.serialize(&buf) orelse unreachable;
    try std.testing.expectEqualStrings("Test\\|Name|S:1|1,1", serialized);
...
test "Engine tick - duplicate region entries (same region twice in playlist)" {
    ...
    // Position 0.02: wrapped around to start (within BOUNDARY_EPSILON), NOW we should advance to entry 1
    action = engine.tick(0.02, 10.0, 0.0, same_region_next, 2, bar_length);
...
test "State toJson with deleted region detection" {
    ...
    regions[0] = .{ .id = 1, .start = 0, .end = 10, .color = 0 };
