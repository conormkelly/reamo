/// Routing Subscriptions - Per-client subscription for track routing data.
///
/// Each client can subscribe to a single track's routing by GUID.
/// When subscribed, the backend pushes routing data (sends, receives, hw outputs) at 5Hz (MEDIUM tier).
///
/// Usage:
///   var subs = RoutingSubscriptions.init(allocator);
///   defer subs.deinit();
...
/// Manages routing subscriptions across multiple clients.
pub const RoutingSubscriptions = struct {
    allocator: Allocator,
    ...
    pub fn init(allocator: Allocator) RoutingSubscriptions {
        var subs = RoutingSubscriptions{
            .allocator = allocator,
            .clients = undefined,
    ...
    pub fn deinit(self: *RoutingSubscriptions) void {
        self.client_id_to_slot.deinit();
    }

    /// Get or create a slot for a client.
    fn getOrCreateSlot(self: *RoutingSubscriptions, client_id: usize) ?usize {
        ...
        if (self.free_count > 0) {
            ...
            self.client_id_to_slot.put(client_id, slot) catch |e| {
                logging.warn("routing_subscriptions: slot reuse failed for client {d}: {}", .{ client_id, e });
                return null;
            };
            return slot;
        }
        ...
        self.next_slot += 1;
        self.client_id_to_slot.put(client_id, slot) catch |e| {
            logging.warn("routing_subscriptions: slot allocation failed for client {d}: {}", .{ client_id, e });
            return null;
        };
        return slot;
    }

    /// Subscribe to a track's routing by GUID.
    pub fn subscribe(self: *RoutingSubscriptions, client_id: usize, track_guid: []const u8) !void {
        ...
        logging.debug("routing_subscriptions: client {d} subscribed to track {s}", .{ client_id, track_guid });
    }

    /// Unsubscribe a client.
    pub fn unsubscribe(self: *RoutingSubscriptions, client_id: usize) void {
        ...
        logging.debug("routing_subscriptions: client {d} unsubscribed", .{client_id});
    }

    /// Remove a client entirely (called on disconnect).
    pub fn removeClient(self: *RoutingSubscriptions, client_id: usize) void {
        const slot = self.client_id_to_slot.get(client_id) orelse return;
        const client = &self.clients[slot];
        client.clear();
        ...
        logging.debug("routing_subscriptions: client {d} removed", .{client_id});
    }

    /// Check if there are any active subscriptions.
    pub fn hasSubscriptions(self: *const RoutingSubscriptions) bool {
    ...
    pub fn activeSubscriptions(self: *RoutingSubscriptions) SubscriptionIterator {
        return .{
            .subs = self,
    ...
    pub fn consumeForceBroadcast(self: *RoutingSubscriptions, slot: usize) bool {
    ...
    pub fn checkChanged(self: *RoutingSubscriptions, slot: usize, data_hash: u64) bool {
        if (slot >= MAX_CLIENTS) return false;
...
test "RoutingSubscriptions init and deinit" {
