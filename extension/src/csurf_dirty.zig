/// CSurf Dirty Flags - tracks what state changed via CSurf callbacks.
///
/// CSurf callbacks signal WHAT changed, dirty flags record it for two purposes:
/// 1. Force immediate broadcast even if hash-based change detection says "unchanged"
///    (provides instant latency response to user actions like fader moves)
/// 2. Detect drift when hash changes without dirty flag (undo/redo, FX drag, etc.)
///
...
/// Result of consuming track dirty flags - bitset + overflow flag.
pub const TrackDirtyResult = struct {
    bits: std.StaticBitSet(MAX_TRACKS),
    all: bool,
};
...
pub const DirtyFlags = struct {
    ...
    pub fn consumeTrackDirty(self: *@This()) TrackDirtyResult {
        const result = TrackDirtyResult{ .bits = self.track_dirty, .all = self.all_tracks_dirty };
    ...
    pub fn consumeFxDirty(self: *@This()) TrackDirtyResult {
        const result = TrackDirtyResult{ .bits = self.fx_dirty, .all = self.all_tracks_dirty };
        self.fx_dirty = std.StaticBitSet(MAX_TRACKS).initEmpty();
        return result;
    }

    /// Consume sends dirty flags - returns current state and clears.
    pub fn consumeSendsDirty(self: *@This()) TrackDirtyResult {
        const result = TrackDirtyResult{ .bits = self.sends_dirty, .all = self.all_tracks_dirty };
